 ...
 * PhaseRetrievalBase::initialise_estimate() method can be used to set up an
 ...
 * required. The PhaseRetrievalBase::set_custom_algorithm allows the
 ...
 * PhaseRetrievalBase::iterate() method.  This is automatically update the
 * values in the Complex_2D estimate and the current error. The error
 * can be retrieved using PhaseRetrievalBase::get_current_error(). The
...
class PhaseRetrievalBase{
 ...
 public:
  ...
  PhaseRetrievalBase(Complex_2D * complex);
  ~PhaseRetrievalBase();
