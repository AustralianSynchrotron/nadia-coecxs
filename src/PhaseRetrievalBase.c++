...
map<string,int> * PhaseRetrievalBase::algNameMap = PhaseRetrievalBase::set_up_algorithm_name_map();

//PhaseRetrievalBase::AlgNameMap[0] = 
// = map_list_of("ER",ER)("BIO",BIO)("BOO",BOO)("HIO",HIO)("DM",DM)("SF",SF)("ASR",ASR)("HPR",HPR)("RAAR",RAAR);

map<string,int> * PhaseRetrievalBase::set_up_algorithm_name_map(){
...
PhaseRetrievalBase::PhaseRetrievalBase(Complex_2D * initial_guess){
...
PhaseRetrievalBase::~PhaseRetrievalBase(){
  delete fft;
  ...
  cout << "In the PhaseRetrievalBase destructor" << endl;

}

double ** PhaseRetrievalBase::get_intensity_autocorrelation(){
...
void PhaseRetrievalBase::initialise_estimate(int seed){
...
void PhaseRetrievalBase::set_support(double ** object_support){
  for(int i=0; i< complex->get_size_x(); i++){
...
void PhaseRetrievalBase::set_intensity(double ** detector_intensity){
  for(int i=0; i< complex->get_size_x(); i++){
    for(int j=0; j< complex->get_size_y(); j++){
...
/**double PhaseRetrievalBase::calculate_error(bool error){
  do_error = error;
  }**/

double PhaseRetrievalBase::get_error(){
...
void PhaseRetrievalBase::project_support(Complex_2D * c){
  int nx = c->get_size_x();
  int ny = c->get_size_y();
...
void PhaseRetrievalBase::project_intensity(Complex_2D * c){
...
void PhaseRetrievalBase::scale_intensity(Complex_2D * c){
...
void PhaseRetrievalBase::set_algorithm(int alg){
  if(algorithm==alg){
...
void PhaseRetrievalBase::set_custom_algorithm(double m1, double m2, double m3, 
...
void PhaseRetrievalBase::print_algorithm(){
...
int PhaseRetrievalBase::iterate(){
